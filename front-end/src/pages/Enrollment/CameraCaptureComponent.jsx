...
import Button from '../../components/CommonButton'; // Assuming you have Button in your ui components
...
const CameraCaptureComponent = ({
  ...
  instructionImage,
  aspectRatio = "4/3",
  photoType = "customer" // Added prop to differentiate between "customer" and "agent"
}) => {
  ...
  const [hints, setHints] = useState(photoType === "customer" ? 'Please align your face' : 'Please position the ID clearly');
  const title = photoType === "customer" ? "Upload Live Photo" : "Capture Agent Photo"; // Dynamic title
  const storageKey = photoType === "customer" ? "customerPhoto" : "agentPhoto"; // Dynamic storage key
  ...
  useEffect(() => {
    ...
    if (photoType === "customer") { // Only load model for customer photo
      loadModel();
    }
  ...
  }, [photoType]); // Dependency on photoType
  ...
  const analyzeFrame = () => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      if (photoType === "customer") { // Only apply skin tone check for customer
    ...
    return {
      hasFace: photoType === "customer" ? skinToneRatio > 0.05 : true, // No face check for agent
  ...
  const getHints = (hasFace, lightingOk, count) => {
    if (photoType === "customer") {
      ...
      if (!lightingOk) return 'Ensure the ID is well-lit and clearly visible.';
      return 'Ready to capture agent photo';
    }
  };
  ...
  const capture = () => {
    ...
    setImgSrc(imageSrc);
    localStorage.setItem(storageKey, imageSrc); // Store in local storage
  ...
  const allValid = validation.hasFace && validation.lightingOk && (photoType === "customer" ? validation.singlePerson : true); // singlePerson check only for customer
            ...
            {!imgSrc ? (
              isCameraActive ? (
                <div className="relative" style={{ aspectRatio }}>
                  ...
                  />
                  {/* Face guide overlay */}
                  {photoType === "customer" && ( // Show guide only for customer
          ...
          <div className="w-full bg-gray-50 p-4 rounded-lg">
            ...
            <ul className="space-y-2">
              <li className={`flex items-center ${validation.hasFace ? 'text-green-600' : 'text-red-600'}`}>
                ...
                {photoType === "customer" ? "Face clearly visible" : "ID card clearly visible"}
              </li>
