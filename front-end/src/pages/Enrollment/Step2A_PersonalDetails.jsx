...
function PersonalDetailsForm({ formData, updateFormData, onNext, onBack }) {
  ...
  const validateForm = () => {
    ...
    const requiredFields = [
      ...
      'religion', 'caste', 'marital_status', 'mobile', 'email',  
    ];
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-2 gap-3">
          ...
          <div>
            <CommanSelect
              ...
              required={true}
              className={validationErrors.marital_status && touchedFields.marital_status ? 'border-red-500' : ''}
            />
            {validationErrors.marital_status && touchedFields.marital_status && (
              <p className="text-red-500 text-xs">{validationErrors.marital_status}</p>
            )}
          </div>
