...
const ImageCaptureValidator = ({
  ...
  const [isCapturing, setIsCapturing] = useState(false); // State to prevent multiple captures
  ...
  // Capture image
  const capture = async () => {
    if (!webcamRef.current || !isWebcamReady || isCapturing) {
      console.log("Capture prevented:", { 
    ...
    try {
      ...
      if (!imageSrc) {
        throw new Error("Failed to capture image: imageSrc is null");
      }

      // Directly create blob and file here, ensuring they are ready before onCapture
      const blob = dataURLtoBlob(imageSrc);
      if (!blob) { // Added explicit check for blob creation
          throw new Error("Failed to create image blob from screenshot.");
      }
      const file = blob; // file is now guaranteed to be a Blob if no error
      ...
      setImgSrc(imageSrc); // Now set imgSrc after the file is created for consistency

      if (onCapture) {
          onCapture(capturedData); // This will now always have `file` if no error occurred
      }

      stopCamera();
    } catch (error) {
      console.error("Capture error:", error);
      setWebcamError(error.message || "Failed to capture or process image. Please try again.");
      setIsCameraActive(true); // Keep camera active on error
      setIsWebcamReady(true);
      setImgSrc(null); // Clear preview on error
    } finally {
  ...
  const retake = () => {
    setImgSrc(null);
    startCamera();
    setIsCapturing(false); // Ensure capturing state is reset on retake
    setIsLoading(false); // Ensure loading state is reset on retake
  };
  ...
  const handleFileUpload = (e) => {
    ...
    setIsCapturing(true); // Treat file upload as a 'capture' process for disabling buttons
    setIsLoading(true); // Show loading state for file upload
    ...
    reader.onload = (event) => {
      ...
      setIsCapturing(false); // Re-enable buttons after file upload
      setIsLoading(false);
    };
    reader.onerror = () => {
        console.error("File read error");
        setWebcamError("Failed to read file.");
        setIsCapturing(false);
        setIsLoading(false);
    };
  ...
  }, [hasExistingPhoto]); // Dependency array to re-run when hasExistingPhoto changes
        ...
        <div className="flex-1">
            ...
            {imgSrc ? (
              ...
              >
                {!isWebcamSupported() ? (
                  <>
                    ...
                    <label className="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-6 rounded-lg cursor-pointer">
                      ...
                      <input
                        ...
                        disabled={isCapturing || isLoading} // Disable during file upload
                      />
                    </label>
                  </>
                ) : (
                  <button
                    onClick={startCamera}
                    disabled={isLoading || isCapturing} // Disable if already loading or capturing
          ...
          <div className="mt-4">
            {imgSrc ? (
              ...
              <button
                onClick={capture}
                disabled={!isWebcamReady || isCapturing || isLoading}
                className="w-full py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium disabled:opacity-50"
              >
                {isCapturing || isLoading ? (
                  <>
              ...
              !isWebcamSupported() && (
                <label className="w-full block py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium text-center cursor-pointer">
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={handleFileUpload}
                    disabled={isCapturing || isLoading} // Disable upload during processing
                  />
                </label>
              )
            )}
