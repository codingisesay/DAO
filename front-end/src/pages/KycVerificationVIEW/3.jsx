    ...
    useEffect(() => {
        ...
        const processDocument = async (doc) => {
            try {
                ...
                // Only update if we actually found something in this document
                const signaturesInThisDoc = apiResponse.data.detections
                    ?.filter(d => d.class_id === 2)
                    .map(d => ({ ...d, image: d.crop })) || [];
                
                const photographsInThisDoc = apiResponse.data.detections
                ...
                setExtractionResults(prev => ({
                    ...prev,
                    [doc.id]: {
                        signatures: signaturesInThisDoc,
                        photographs: photographsInThisDoc,
                        status: signaturesInThisDoc.length > 0 || photographsInThisDoc.length > 0 ? 'verified' : 'failed'
                    }
                }));
...
const DocumentDetailsTable = ({ documentslist, extractionResults, isProcessing }) => {
    ...
    const renderExtractedItems = (items, docId, type) => {
        // Check if we have results for this specific document
        ...
        <div className="p-4 max-w-4xl mx-auto">
            ...
            {Object.entries(groupedDocs).map(([type, docs]) => (
                <div key={type} className="mb-8">
                    <h2 className="text-xl font-bold mb-4 capitalize">{toTitleCase(type)}</h2>
